...
def parse_processor_data(specs, category_name, name, price, avail, url):
...
def parse_motherboard_data(specs, category_name, name, price, avail, url):
...
def parse_ram_data(specs, category_name, name, price, avail, url):
  memory_type = specs.get('Atmiņas tips')
  if memory_type:
    memory_type = memory_type.strip()
  return {
...
def parse_gpu_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_ssd_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_hdd_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_case_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_fan_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_psu_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def parse_cooler_data(specs, category_name, name, price, avail, url):
  return {
    'category': category_name,
    'name': name,
    'url': url,
...
def scrape_page(url, page_num, category_name, product_type, conn):
  ...
  for component in components:
    try:
      ...
      product_url = 'https://www.dateks.lv' + product_link['href'] if product_link else None
      ...
      if product_type == 'processors':
        ...
        product_data = parse_motherboard_data(specs, category_name, name, price, avail, product_url)
        success = database.save_motherboard(conn, product_data)
      elif product_type == 'ram':
        product_data = parse_ram_data(specs, category_name, name, price, avail, product_url)
        success = database.save_ram(conn, product_data)
      elif product_type == 'gpus':
        product_data = parse_gpu_data(specs, category_name, name, price, avail, product_url)
        success = database.save_gpu(conn, product_data)
      elif product_type == 'ssd':
        product_data = parse_ssd_data(specs, category_name, name, price, avail, product_url)
        success = database.save_ssd(conn, product_data)
      elif product_type == 'hdd_35':
        product_data = parse_hdd_data(specs, category_name, name, price, avail, product_url)
        ...
        product_data = parse_case_data(specs, category_name, name, price, avail, product_url)
        success = database.save_case(conn, product_data)
      elif product_type == 'fans':
        product_data = parse_fan_data(specs, category_name, name, price, avail, product_url)
        success = database.save_fan(conn, product_data)
      elif product_type == 'psu':
        product_data = parse_psu_data(specs, category_name, name, price, avail, product_url)
        success = database.save_psu(conn, product_data)
      elif product_type == 'coolers':
        product_data = parse_cooler_data(specs, category_name, name, price, avail, product_url)
        success = database.save_cooler(conn, product_data)
      else:
        success = False
